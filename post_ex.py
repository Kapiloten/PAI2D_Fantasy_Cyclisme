...
# nombre de points gagnés par un coureur p pour une période t
v = pulp.LpVariable.dicts("v", (P, E), cat=pulp.LpBinary)    # 0/1
...
y = pulp.LpVariable.dicts("y", (P, E), cat=pulp.Lp.Binary)
...
for i in range(1, len(T)):
    t_1 = T[i-1]
    t = T[i]
    prob += (pulp.lpSum(c[p][t] * (x[p][t] - x[p][t_1]) for p in P) == r[t_1] - r[t]), f"Budget_de_{t_1}_à_{t}"
